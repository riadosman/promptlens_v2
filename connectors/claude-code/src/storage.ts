...
type State = { queue: QueuedPrompt[] };

export class SecureConnectorStorage implements ConnectorStorage {
  private readonly statePath = join(homedir(), '.promptlens', 'claude-code', 'state.json');
  private readonly keyring = new Entry('PromptLens', 'claude-code');
  ...
  async loadQueue(): Promise<readonly QueuedPrompt[]> {
    return this.withLock(async () => (await this.readState()).queue);
  }

  async saveQueue(queue: readonly QueuedPrompt[]): Promise<void> {
    await this.withLock(async () => this.writeState({ queue: [...queue] }));
  }
