...
describe('Claude Code hook installer', () => {
  it('installs an absolute, idempotent capture command', async () => {
    temporaryHome = await mkdtemp(join(tmpdir(), 'promptlens-claude-hook-'));
    process.env.HOME = temporaryHome;
    process.env.USERPROFILE = temporaryHome;
    await installHook();
    await installHook();
    const settings = JSON.parse(
      await readFile(join(temporaryHome, '.claude', 'settings.json'), 'utf8'),
    ...
    expect(commands[0]).toMatch(/claude-code.+src.+cli\.js.+capture/u);
